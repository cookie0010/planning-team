...
description: canvas UX 디자인 — HTML 와이어프레임 전환, 공용 컴포넌트(wf.css/wf-screen), 화면 결정, 디바이스·권한 모델
metadata:
  type: project
---

# canvas — UX 디자인 (와이어프레임)
...
**How to apply:** 재개 시 `canvas/prd/prd-v9.md`(확정 스펙) + `ia/ia-v8.md`(최신 IA) + `wireframe/`(HTML)부터. (2026-06-17 경로 변경: PRD→`prd/`, IA→`ux/` 밖 최상위 `ia/`, one-pager→`one-pager/`) 화면은 IA 화면 목록과 1:1, 행동은 PRD 기능에 추적. ⚠️**이 HTML 와이어프레임은 메인 세션이 직접 그렸음**(ux-designer 에이전트 호출 아님) — 산출물은 wireframe/에 있음.

## ★ Figma → HTML 전환 (중요)
- 박스 와이어프레임을 **HTML로** `canvas/wireframe/`에 그린다. (구 Figma 박스 v1 fileKey=zjIQhVBjkvjCHUM1Npqcpe는 구버전.)
...
- **로컬 미리보기·검증**: wireframe/에서 no-cache 정적서버(`python3 -c "...Cache-Control: no-store..."` 8753) — 외부 JS/CSS 캐시 안 갱신 문제 해결책. 자산 링크 `?v=N`. 검증=playwright MCP + http://localhost:8753/ (file:// 차단).
