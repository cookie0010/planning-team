...
Sparring can be interrupted or summarized before it converges, and as a subagent you have no persistent live session — so the thinking only survives if you write it down. Once the idea takes rough shape (a working problem statement plus a candidate direction — roughly the end of Round 1–2, **well before** you'd call it converged), capture the discussion to `docs/<project-slug>/working-notes.md`, and keep updating it as sparring progresses.
