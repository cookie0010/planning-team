...
현재 진행 프로젝트 = `canvas` (slug). 아래 `<slug>`는 프로젝트별 폴더명.
...
아이디어
  ...
  ▼ <slug>/idea-deep-dive.md
@one-pager-writer — 한 장 설득문서로 압축
  ▼ <slug>/one-pager/one-pager.md
...
| `ux-designer` (+ `wireframe-css` 스킬) | 검증된 IA를 **HTML 박스 와이어프레임**으로 (Figma 아님) | PRD + 검증된 IA (+ benchmark·reference) | `wireframe/*.html` + `ux/wireframe-notes-v<N>.md` |
...
├── wireframe/                   # ux-designer HTML 와이어프레임 (standalone .html + wf.css + 공용 wf-screen.css/js) — ※ ux 밖 최상위
├── ux/
│   ├── benchmark-v<N>.md        # IA·와이어프레임 설계 입력 (선택 — 필수 아님)
│   ├── references/              # UI 참고 캡처
│   └── wireframe-notes-v<N>.md  # 와이어프레임 설계 노트
...
- **`template/`** = 에이전트가 **읽는** 틀 (one-pager / prd / idea-deep-dive 템플릿). 산출물 아님.
- **`references/`** (저장소 루트) = wf.css 원본 + 와이어프레임 예시 (wireframe-css 스킬 원천). `<slug>/`와 무관한 공용 자산.
...
- **버전 덮어쓰기 금지**: 모든 주 산출물은 새 파일로 추가(이전 버전 보존). 문서 내 `Version:`과 파일명 동기화. (1-pager·PRD·IA·와이어프레임 모두.)
- **PRD·IA 버전 네이밍 (2026-06-18 정리)**: `vN.n` 형식으로 기록한다 (`prd-v0.15.md`, `ia-v0.13.md` 등). **`0.n`(0.1, 0.2 … 0.15 …) = 아직 사용자 검증 전 프리릴리스 단계**, 사용자가 "확정" 선언할 때 비로소 `1.0`으로 올린다. 이후 마이너 `1.1`·큰 개정 `2.0`. **1.0을 성급히 붙이지 말 것** — canvas도 한때 v1.0/v1.1로 올렸다가 "아직 1.0은 무리"라는 사용자 판단으로 `prd-v0.15`/`ia-v0.13`(0.x 프리릴리스)로 환원함. (과거 단순 `-v<N>` 파일은 그대로 두고 소급 안 함.)
...
위는 설계된 이상적 파이프라인이다. 실제로는 **메인 세션이 직접 처리한 단계**가 있다 — 특히 canvas의 **HTML 와이어프레임은 @ux-designer 에이전트가 아니라 메인 세션이 직접 그렸고**, prd-v2도 사용자 지시로 메인 세션이 한 번 직접 썼다. 에이전트는 "이렇게 일하도록 정의된 일꾼"이고, 메인 세션이 같은 산출물 규칙(경로·버전·검증)을 따라 직접 처리하기도 한다.
